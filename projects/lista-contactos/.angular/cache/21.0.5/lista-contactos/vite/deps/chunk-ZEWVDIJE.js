...
function addServerStyles(doc, appId, inline, external) {
  var _a2;
  const elements = (_a2 = doc.head) == null ? void 0 : _a2.querySelectorAll("style[".concat(APP_ID_ATTRIBUTE_NAME, '="').concat(appId, '"],link[').concat(APP_ID_ATTRIBUTE_NAME, '="').concat(appId, '"]'));
  if (elements) {
...
function warningOptionsMessage(req) {
  if (req.credentials && req.withCredentials) {
    console.warn(formatRuntimeError(2819, "Angular detected that a `HttpClient` request has both `withCredentials: true` and `credentials: '".concat(req.credentials, "'` options. The `withCredentials` option is overriding the explicit `credentials` setting to 'include'. Consider removing `withCredentials` and using `credentials: '").concat(req.credentials, "'` directly for clarity.")));
  }
}
...
function validateXhrCompatibility(req) {
  ...
  for (const {
    ...
    if (req[property]) {
      console.warn(formatRuntimeError(errorCode, "Angular detected that a `HttpClient` request with the `".concat(property, "` option was sent using XHR, which does not support it. To use the `").concat(property, "` option, enable Fetch API support by passing `withFetch()` as an argument to `provideHttpClient()`.")));
    }
  }
}
var _HttpXhrBackend = class _HttpXhrBackend {
  ...
  maybePropagateTrace(fn) {
    var _a2;
    return ((_a2 = this.tracingService) == null ? void 0 : _a2.propagate) ? this.tracingService.propagate(fn) : fn;
  }
  handle(req) {
    if (req.method === "JSONP") {
...
__publicField(_HttpBackend, "ɵprov", ɵɵdefineInjectable({
  token: _HttpBackend,
...
__publicField(_HttpHandler, "ɵprov", ɵɵdefineInjectable({
  token: _HttpHandler,
...
__publicField(_HttpXsrfTokenExtractor, "ɵprov", ɵɵdefineInjectable({
...
function makeHttpResourceFn(responseType) {
  return function httpResource2(request, options) {
    ...
    return new HttpResourceImpl(injector, () => normalizeRequest(request, responseType), options == null ? void 0 : options.defaultValue, options == null ? void 0 : options.debugName, options == null ? void 0 : options.parse, options == null ? void 0 : options.equal);
  };
}
...
//# sourceMappingURL=chunk-ZEWVDIJE.js.map

...
function checkFullData(data) {
  if (!data[LocaleDataIndex.ExtraData]) {
    throw new RuntimeError(2303, ngDevMode && 'Missing extra locale data for the locale "'.concat(data[LocaleDataIndex.LocaleId], '". Use "registerLocaleData" to load new data. See the "I18n guide" on angular.io to know more.'));
  }
}
...
function assertValidDateFormat(parts) {
  if (parts.some((part) => /^Y+$/.test(part)) && !parts.some((part) => /^w+$/.test(part))) {
    const message = 'Suspicious use of week-based year "Y" in date pattern "'.concat(parts.join(""), '". Did you mean to use calendar year "y" instead?');
    if (parts.length === 1) {
...
function getDatePart(part, date) {
  switch (part) {
    ...
    case 7:
      return date.getDay();
    default:
      throw new RuntimeError(2301, ngDevMode && 'Unknown DateType value "'.concat(part, '".'));
  }
}
...
function getDateTranslation(date, locale, name, width, form, extended) {
  switch (name) {
    ...
    default:
      ...
      throw new RuntimeError(2302, ngDevMode && "unexpected translation type ".concat(unexpected));
  }
}
function timeZoneGetter(width) {
  return function(date, locale, offset) {
    ...
    switch (width) {
      ...
      default:
        throw new RuntimeError(2310, ngDevMode && 'Unknown zone width "'.concat(width, '"'));
    }
  };
}
var JANUARY = 0;
...
function toDate(value) {
  ...
  if (!isDate(date)) {
    throw new RuntimeError(2311, ngDevMode && 'Unable to convert "'.concat(value, '" into a date'));
  }
  return date;
}
...
function formatNumberToLocaleString(value, pattern, locale, groupSymbol, decimalSymbol, digitsInfo, isPercent = false) {
  ...
  if (!isFinite(value)) {
    ...
    if (digitsInfo) {
      ...
      if (parts === null) {
        throw new RuntimeError(2306, ngDevMode && "".concat(digitsInfo, " is not a valid digit info"));
      }
...
function roundNumber(parsedNumber, minFrac, maxFrac) {
  if (minFrac > maxFrac) {
    throw new RuntimeError(2307, ngDevMode && "The minimum number of digits after fraction (".concat(minFrac, ") is higher than the maximum (").concat(maxFrac, ")."));
  }
  let digits = parsedNumber.digits;
...
function getPluralCategory(value, cases, ngLocalization, locale) {
  ...
  throw new RuntimeError(2308, ngDevMode && 'No plural message found for value "'.concat(value, '"'));
}
...
var _NgForOf = class _NgForOf {
  ...
  set ngForTrackBy(fn) {
    if ((typeof ngDevMode === "undefined" || ngDevMode) && fn != null && typeof fn !== "function") {
      console.warn("trackBy must be a function, but received ".concat(JSON.stringify(fn), ". ") + "See https://angular.dev/api/common/NgForOf#change-propagation for more information.");
    }
    this._trackByFn = fn;
  }
  get ngForTrackBy() {
...
function strToNumber(value) {
  ...
  if (typeof value !== "number") {
    throw new RuntimeError(2309, ngDevMode && "".concat(value, " is not a number"));
  }
  return value;
}
...
function assertValidDecodingInput(dir) {
  ...
  if (typeof dir.decoding === "string" && !validInputs.includes(dir.decoding)) {
    throw new RuntimeError(2952, "".concat(imgDirectiveDetails(dir.ngSrc), " the `decoding` attribute ") + "has an invalid value (`".concat(dir.decoding, "`). ") + 'To fix this, provide a valid value ("sync", "async", or "auto").');
  }
}
...
//# sourceMappingURL=chunk-576LAPVC.js.map
